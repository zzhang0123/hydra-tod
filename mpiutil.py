...
def parallel_map_gather(func, glist, multi_inputs=False, root=None, method="con", comm=_comm, njobs=cpu_affinity):
    """
    ...
    if comm is None or comm.size == 1:
        if multi_inputs:
            return Parallel(n_jobs=njobs)(delayed(func)(*item) for item in glist)
        else:
            return Parallel(n_jobs=njobs)(delayed(func)(item) for item in glist)
...
def parallel_map_sum(func, glist, multi_inputs=False, root=None, method="con", comm=_comm, njobs=cpu_affinity):
    """
    Apply a parallel map using MPI.
    Should be called collectively on the same list. All ranks return the full
    set of results.
    ...
    if comm is None or comm.size == 1:
        if multi_inputs:
            ...
            result = Parallel(n_jobs=njobs)(delayed(func)(*item) for item in glist)
        else:
            result = Parallel(n_jobs=njobs)(delayed(func)(item) for item in glist)
